...
def createSpaceTimeGraph(G_list, time_window, start_node, time_step):
    ...
    for time1 in range(1, time_window + 1):    # 在一定时间窗口内
        past_node = start_node.split("_")[0] + "_" + str(time_step - time1)   # 新建一个过去时间间隔内的该节点的多个副节点，加_4;+3等区分
        ...
        if past_node not in list(G_list[time_window+1 - time1].nodes()):  # if start_node not in G_list:
            ...
            G.add_edge(start_node, past_node)  # 将新老时刻节点间建立连边
...
def random_walk(SpaceTimegraph, path_length, rand=random.Random(0), start=None):
    ...
    while len(path) < path_length:
        cur = path[-1]  # 当前是路径中最后一个
        if len(G[cur]) > 0:
            path.append(rand.choice(list(G[cur])))  # 这个代码为什么是从当前节点邻居中选一个
        else:
            break
    return path


# 这是作者的新东西，关键是在这个时间图中如何产生游走序列walks，后面的问题就是直接采用word2vec了；
...
if __name__ == '__main__':
    ...
    if not os.path.exists(direc+"/output_stwalkone"):  # 如果不存在就创建这一目录
