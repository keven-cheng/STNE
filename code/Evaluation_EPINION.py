...
__date__ = '2018/8/27 上午10:38'
__product__ = 'PyCharm'
__filename__ = 'Evaluation_EPINION'
...
def readNetworkData(dir, stemmer=0):#dir, directory of network dataset
    ...
    allvectors = []
    labelset = set()
    with open(dir + '/labels_104.txt') as f1, open(dir + '/labels_109.txt') as f2:
...
def text_save(filename, data):
    ...
    for i in data:
        ...
        label = i[2]  # 用户标签
        time = i[3]    # 标记时刻
        s = tags[0]+' '+label.replace("\n", '')+' '+str(time)+'\n'  # 原来的文件label后面有一个换行，去掉后文末再加一个；
        file.write(s)
    file.close()
    print('%s标签文件保存成功',filename)
...
if __name__ == '__main__':
    directory = '../LABELS/EPINION' #data directory
    ...
    # text_save('EPINION_label.txt', vector_list)
