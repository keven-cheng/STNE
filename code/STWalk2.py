...
def createSpaceTimeGraph(G_list, time_window, start_node, time_step):
    ...
    for time1 in range(1, time_window + 1):
        ...
        # 修改错误同STwalk1
        if past_node not in list(G_list[- time1-1].nodes()):
            continue
        else:
...
def combineEmbedding(spaceFilePath, timeFilePath, time_step, output_file):
    ...
    timeDict = dict()
    finalEmb = dict()
#TODO 暂时对于DBLP数据集不适用，因为那个中是人名字，空格分开的不合适，并且下面程序中用if 含有——和时间戳也不相符
    with open(spaceFilePath, "r") as inFile:
        for line in inFile:
            #inFile.readline()  # 多余部分
            token = line.strip().split(" ")  #根据空格进行切分
            if "_" + str(time_step) in token[0]:  #如果第一列中带有时间戳，将其处理存入空间字典中
                # TODO 修改原程序中错误1，之前req=token[1]中少加冒号，导致表示只有第一行
                # rep = token[1].strip().split(" ")
                rep = token[1:]
                spaceDict[token[0]] = [rep[i] for i in range(0, len(rep))]  #将嵌入表示数据存到对应的以空间字典索引的列表中
    ...
    with open(timeFilePath, "r") as inFile:
        for line in inFile:
            ...
            if "_" + str(time_step) in token[0]:
                #TODO 修改原程序中错误2，之前req=token[1]中少加冒号，导致表示只有第一行
                # rep = token[1].strip().split(" ")
                rep = token[1:]
...
#  两种嵌入表示的合并部分
    for spaceKey in spaceDict:
        if spaceKey in timeDict:
            finalEmb[spaceKey] = []
            for i in range(len(spaceDict[spaceKey])):   # ？？？同时计算结果出的嵌入表示只有一列，应该是少算了，少了一个循环
                lst = finalEmb[spaceKey]
                spacelist = spaceDict[spaceKey]
                timelist = timeDict[spaceKey]
                # TODO 修改原程序中不足3，使结果只保留小数点后6位
                lst.append(float('%.6f'%(float(spacelist[i]) + float(timelist[i]))))  # 为什么会将两者直接相加完事呢，原来程序有错误；修改增加只保留后6位
                finalEmb[spaceKey] = lst
    #TODO 为了方便结果评测，输出的文件中最好在末尾加上用户标签；
