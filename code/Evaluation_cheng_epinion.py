...
__date__ = '2018/12/19 上午9:42'
__product__ = 'PyCharm'
__filename__ = 'Evaluation_cheng_epinion'
...
def readEmbeddingData(dir, labelDict, stemmer=0):
    ...
    Embeddings = []
    count = 0
    with open(dir+'/spatiotemporal_4.stwalkone') as f1:   # spatiotemporal_window_4.stwalktwo
        for l1 in islice(f1, 1, None):      # 数字1控制从第几行开始读取，islice(seq,start,stop,step)
            ...
            index = len(tags)
            tag = tokens[0]      # 对于ciao数据集，用户名形式为1344_4 ，需要去掉后面的时间段标记信息；根据_进行切分，取前半段
            if tag in labelDict:
...
if __name__ == '__main__':
    directory_label = '../LABELS/EPINION_label.txt'              # data directory
    directory_embedding = '../epinion/output_stwalkone'          # 嵌入表示数据位置
